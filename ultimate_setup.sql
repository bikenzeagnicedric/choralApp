...
-- Run this to fix ALL issues (Missing tables + Policy conflicts)
...
-- 2. Create USER_FAVORITES table if missing
...
-- 4. APPLY RLS POLICIES (with clean up)
...
drop policy if exists "Everyone can view songs" on public.songs;
...
drop policy if exists "Everyone can view masses" on public.masses;
...
-- 5. Helper: If you are the only user, make yourself admin (Optional, un-comment to use)
